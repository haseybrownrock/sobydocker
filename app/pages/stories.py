...
if st.button("おはなしを生成する") and user_message:
    with st.spinner('お話を生成中...'):
        try:
            ...
            new_record = TextRecord(input_text=user_message, output_text=st.session_state["ai_text"])
            session.add(new_record)
            session.commit()
            session.close()
        except openai.error.InvalidRequestError as e:
            if e.code == 'content_policy_violation':
                st.error("このおはなしはつづけられないよ。別のお話にしてね")
            else:
...
if st.session_state["show_story"]:
    ...
    if st.button("お話を閉じる"):
        ...
        st.rerun()  # セッション状態を即座に反映するためにリロード
